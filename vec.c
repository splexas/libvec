...
vec_t *vec_init(int elem_size, int reserved_elements)
{
    ...
    vec->reserved = reserved_elements != -1 ? 
        vec->elem_size * reserved_elements :
        vec->elem_size * VEC_RESERVED_ELEMENTS;
...
void vec_remove(vec_t *vec, unsigned int index)
{
    ...
    for (int i = index; i < vec->elem_count; i++)
        memcpy(vec->arr + vec->elem_size * i,
        vec->arr + vec->elem_size * (i + 1), vec->elem_size);
...
{
    for (int i = 0; i < vec->elem_count; i++) {
        if (memcmp(vec->arr + (vec->elem_size * i), val, vec->elem_size) == 0)
            return i;
    }
    return -1;
}
